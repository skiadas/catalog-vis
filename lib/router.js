...
function parseHash() {
  ...
  if (parts[0] === 'schedule') {
    ...
    if (view === 'slot' && parts[2] && parts[3]) {
      return { view: 'schedule', params: { scheduleView: 'slot', day: parts[2], time: decodeURIComponent(parts[3]) } }
    }
    ...
    if (view === 'instructor' && parts[2]) {
      return { view: 'schedule', params: { scheduleView: 'instructor', instructor: decodeURIComponent(parts[2]) } }
    }
...
export function navigate(view, params = {}) {
  ...
  if (view === 'programs') hash = '#/'
    ...
    else if (params.scheduleView === 'slot') hash = '#/schedule/slot/' + params.day + '/' + encodeURIComponent(params.time)
    else if (params.scheduleView === 'course') hash = '#/schedule/course/' + encodeURIComponent(params.code)
    else if (params.scheduleView === 'instructor') hash = '#/schedule/instructor/' + encodeURIComponent(params.instructor)
  }
