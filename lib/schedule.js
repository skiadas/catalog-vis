  ...
  for (const otherCode of Object.keys(index.byCourse)) {
    ...
    for (const m of mine) {
      for (const theirs of theirSections) {
        if (conflictsBetween(m, theirs)) { conflicted = true; break }
      }
      if (conflicted) break
    }
    if (conflicted) {
...
const DEPT_PALETTE = [
  '#1b4965', '#e67e22', '#7b2d8b', '#00695c', '#c62828', '#0288d1', '#8e24aa', '#2e7d32',
  '#fb8c00', '#5d4037', '#d81b60', '#3949ab', '#00897b', '#6a1b9a', '#ef6c00', '#7cb342',
  '#4527a0', '#ad1457', '#1565c0', '#558b2f'
]
