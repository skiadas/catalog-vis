...
export function setSelectedSchedules(ids) {
  selectedScheduleIds.value = [...ids]
  persistSelectedSchedules()
}
...
// Stable identity for a schedule offering (code + section), used to locate the
// exact offering during a drag-and-drop reschedule.
export function offeringKey(id, prefix, number, section) {
  return `${id}|${prefix}|${number}|${section}`
}
...
export async function loadData() {
  try {
    const [majorsRes, parsedRes, coreRes, baseRes] = await Promise.all([
      fetch('majors.json'),
      fetch('requirements_parsed.json'),
      fetch('core_requirements.json'),
    ...
    seedSchedules([{ id: 'base', name: 'Sample schedule', offerings: parseCsv(await baseRes.text()) }])
  } catch (err) {
