...
export async function loadData() {
  try {
    ...
    // Seed the collection with a deterministic "Sample schedule" generated from
    // the live catalog (seed 42 for reproducibility); seedSchedules prefers any
    // schedules already saved in localStorage.
    const { facultyByPrefix, eligible } = buildFacultyAndEligible(programs.value, allCourses.value)
    seedSchedules([
      {
        id: 'base',
