...
function assignItems(items, pool, catalog) {
  ...
  const ok = solve(0, [], [])
  if (!ok) return { ok: false, used: [], plans: [] }
  return result
}
...
export function assignRequirement(requirement, taken, catalog) {
  ...
  for (let si = 0; si < sections.length; si++) {
    ...
    ;(s.items || []).forEach((it, i) => {
      ...
      // Electives placeholders are always "ok" from the rigid pass's view (they
      // consume nothing); their real satisfaction is decided by fillElectives.
      const ok = assigned.ok && (plan.type === 'electives' ? true : nodeSatisfied(plan))
      entries.push({
        sectionIndex: si,
        item: it,
        ok,
