...
// Courses that must be reserved for the requirement's non-electives nodes
// (required courses, chosen any_of alternatives, some_of picks, etc.). These
// may not also fill the requirement's electives bucket(s) — a course counts
// for only one requirement node within a track. Across tracks it's fine.
...
export function evaluateRequirement(requirement, taken, catalog) {
  const claimed = claimedCourses(requirement, taken, catalog)
  return {
    label: requirement.label,
    sections: (requirement.sections || []).map((s) => ({
      heading: s.heading,
      items: (s.items || []).map((it) =>
        it.type === 'electives' ? satisfied(it, taken, catalog, claimed) : satisfied(it, taken, catalog),
      ),
    })),
  }
}
...
export function gapGroups(item, taken, catalog, excluded) {
  ...
  switch (item.type) {
    ...
    case 'any_of': {
      ...
      return (item.items || []).flatMap((it) => gapGroups(it, taken, catalog, excluded))
    }
    case 'each_of':
    case 'pair': {
      const items = item.items || (item.codes || []).map((code) => ({ type: 'course', code }))
