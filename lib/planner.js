...
export function gapGroups(item, taken, catalog) {
  const result = satisfied(item, taken, catalog)
  ...
  switch (item.type) {
    ...
    case 'any_of': {
      ...
      return (item.items || []).flatMap((it) => gapGroups(it, taken, catalog))
    }
    case 'each_of':
    case 'pair': {
      const items = item.items || (item.codes || []).map((code) => ({ type: 'course', code }))
      return items.flatMap((it) => gapGroups(it, taken, catalog))
    }
    case 'some_of': {
      const min = item.min ?? 1
      ...
      for (const ch of children) {
        ...
        for (const g of gapGroups(ch, taken, catalog)) {
    ...
    case 'electives': {
      ...
      if (result.count < (item.count || 0)) {
        ...
        const desc = describeConstraints(constraints)
        const weighted = [...result.missing].sort(
          (a, b) => electivesWeight(b, constraints) - electivesWeight(a, constraints) || a.localeCompare(b),
        )
        const scope = desc || 'eligible courses'
        const label = `Need ${need} more ${scope}`
        return [{ label, codes: weighted.slice(0, need), suggested: true }]
      }
      const aggText = describeConstraints((constraints || []).filter((c) => hasCount(c)))
      return [
        {
          note: `Course count is met, but there's still a category rule to satisfy: ${aggText || 'see catalog'}.`,
        },
      ]
    }
    default:
