...
export function describeConstraints(constraints) {
  const parts = []
  for (const c of constraints || []) {
    if (c.type === 'discipline' && c.prefixes && c.prefixes.length) {
...
export function gapGroups(item, taken, catalog, excluded) {
  ...
  switch (item.type) {
    ...
    case 'any_of': {
      ...
      // still-open alternatives as a single choice, each option keeping its
      // own courses together (e.g. a science pair "BIO 161 + BIO 185") instead
      // of flattening them into a bare list.
      const alternatives = []
      for (const alt of item.items || []) {
        if (satisfied(alt, taken, catalog).status === 'satisfied') continue
        const codes = gapGroups(alt, taken, catalog, excluded).flatMap((g) => g.codes || [])
        alternatives.push({
          codes: codes.length ? [...new Set(codes)] : planGaps(alt, taken, catalog).courses,
        })
      }
      if (!alternatives.length) return []
      // When every alternative is a single code, keep the familiar flat form.
      if (alternatives.every((a) => a.codes.length === 1)) {
        return [{ label: item.note || 'Choose one of', codes: alternatives.flatMap((a) => a.codes) }]
      }
