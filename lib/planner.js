...
// True when `code` is satisfied by the `taken` set. Cross-listed/aliased codes
// (ENG/COM 251, GNDR 499) are satisfied by any one of their concrete spellings,
// but a number range (ENV 408-409) is a multi-part sequence that requires every
// course in the range to be taken.
function codeSatisfied(code, takenSet) {
  const expanded = expandCode(code)
  if (/\d{3}-\d{3}$/.test(code)) {
    return expanded.filter((c) => c !== code).every((c) => takenSet.has(c))
  }
  return expanded.some((c) => takenSet.has(c))
}
...
export function satisfied(item, taken, catalog, excluded) {
  ...
  switch (item.type) {
    case 'course': {
      const hit = codeSatisfied(item.code, takenSet)
    ...
    case 'any_of': {
      const results = item.codes
        ? item.codes.map((code) => {
            ...
            const ok = codeSatisfied(code, takenSet)
      ...
      if (item.codes) {
        for (const code of item.codes) {
          const parts = /\d{3}-\d{3}$/.test(code)
            ...
            : [expandCode(code).find((c) => pool.has(c))]
          if (parts.every(Boolean) && parts.every((c) => pool.has(c))) {
            for (const c of parts) pool.delete(c)
            return { ok: true, used: parts, plan: { type: 'choice', chosen: parts } }
          }
        }
...
export function assignRequirement(requirement, taken, catalog) {
  ...
  for (let si = 0; si < sections.length; si++) {
    ...
    for (const it of s.items || []) {
      ...
      entries.push({ sectionIndex: si, item: it, ok: r.ok, plan: r.plan, used: new Set(r.used), pool })
    }
  }
