<template>
  <div class="planner-req">
    <div class="planner-req-top">
      <span class="planner-status" :class="report.status">{{
      ...
      <div class="planner-gap-group" v-for="(g, gi) in report.gaps" :key="gi">
        <template v-if="g.expandable">
          <button class="planner-gap-toggle" @click="toggleExpand(gi)">
            ...
            <span class="planner-gap-count"
              >({{ g.codes.length }} option{{ g.codes.length !== 1 ? 's' : '' }})</span
            >
        ...
        <template v-else-if="g.alternatives">
          ...
          <div v-for="(alt, ai) in g.alternatives" :key="ai" class="planner-gap-alternative">
            <span class="anyof-option">{{ 'Option ' + (Number(ai) + 1) }}</span>
            <template v-for="(slot, si) in alt.slots" :key="si">
              <span v-if="Number(si) > 0" class="pair-plus">+</span>
              ...
              <template v-for="(code, ci) in slot.codes" :key="code">
                <span
                  ...
                  >{{ code }}</span
                >
                <span v-if="slot.label && Number(ci) < (slot.codes || []).length - 1" class="anyof-or"
                  >or</span
                >
              </template>
            </template>
