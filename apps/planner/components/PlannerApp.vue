  ...
  <div>
    <div class="header planner-head">
      ...
      <div class="planner-top-left">
        ...
        />
        <select class="planner-plan-select" :value="currentPlanId" @change="onPlanChange">
    ...
    <div class="planner-top">
      <span class="planner-summary-item"
        >{{ takenCourses.length }} course{{ takenCourses.length !== 1 ? 's' : '' }} planned</span
      >
    ...
    <div class="planner-tabs">
      ...
      <button class="planner-tab" :class="{ active: tab === 'requirements' }" @click="tab = 'requirements'">
    ...
    <div
      ...
      @keydown.esc="showAddCourse = false"
    >
      <div class="modal modal-wide">
        ...
        <div v-else class="planner-pick-hint">
          Search for a course to add it to your plan. It lands on the "unassigned" shelf — drag it into a term
          afterwards.
        </div>
      </div>
    </div>
    ...
    <div
      ...
      @keydown.esc="showAddTrack = false"
    >
      <div class="modal">
        <div class="modal-head">
        ...
        <div class="controls">
          ...
          <div class="filter-group">
            ...
            <button class="filter-btn" :class="{ active: type === 'major' }" @click="type = 'major'">
              Majors
            </button>
            <button class="filter-btn" :class="{ active: type === 'minor' }" @click="type = 'minor'">
              Minors
            </button>
          </div>
        </div>
        ...
        <div class="add-program-list">
          <div class="add-program-row" v-for="p in filteredPrograms" :key="p.id">
            <a v-if="p.id !== CORE_ID" class="add-program-name" :href="browseProgramUrl(p.id)">{{
              p.name
            }}</a>
            ...
            <div class="add-program-chips">
              <span
                ...
                >{{ t.label }}</span
              >
              <span v-if="!filteredTracksFor(p).length" class="track-chip disabled"
                >No structured requirements</span
              >
            </div>
          </div>
        </div>
...
import TrackAudit from './TrackAudit.vue'
...
export default {
  ...
  setup() {
    ...
    const onPlanChange = (e) => loadPlan(e.target.value)
    const query = ref('')
    const type = ref('all')
