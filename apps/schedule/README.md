...
— one per column (a lone course spans the width, two split it, three or more
use thirds and wrap), with wrapped remainders keeping the same width rather
than stretching. A band with more courses than fit **scrolls** inside its card.
Both the week grid and the day column share a **calendar-height** control (Auto
· 1× · 2×, persisted locally): `auto` doubles the whole axis only when the
busiest time band holds more than three courses (the point where
lane-split/stacked rows stop fitting), while `1×`/`2×` pin the base or doubled
...
the first owned schedule (nothing when the user owns none), so a stranger's
schedule is never auto-selected. As a route, the page is deep-linkable and the
browser back button leaves it (a direct visit's "← Back" falls back to the
grid).
