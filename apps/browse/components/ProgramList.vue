<template>
  <div>
    <div class="header">
      ...
      <div class="controls">
        <input class="search-input" type="text" placeholder="Search programs..." v-model="searchQuery" />
    ...
    <div class="program-grid">
      <div class="program-card" v-for="p in filteredPrograms" :key="p.id" @click="goToProgram(p.id)">
        <h3>{{ p.name }}</h3>
        <div class="meta">
          <span class="tag major" v-if="p.type.includes('major')">Major</span>
          <span class="tag minor" v-if="p.type.includes('minor')">Minor</span>
          <span class="tag program" v-if="!p.type.includes('major') && !p.type.includes('minor')"
            >Program</span
          >
        </div>
      </div>
