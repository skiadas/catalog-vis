<template>
  <div v-if="currentProgram">
    ...
    <div v-if="currentParsed && currentParsed.length">
      ...
      <div class="req-block" v-for="(req, ri) in currentParsed" :key="ri">
        ...
        <RequirementSection v-for="(section, si) in req.sections" :key="si" :section="section" />
      </div>
    </div>

    <div v-else-if="Object.keys(currentProgram.requirements).length">
      <div class="section-title">Requirements</div>
      <div class="req-block" v-for="(req, key) in currentProgram.requirements" :key="key">
        <h4>{{ req.label }}</h4>
