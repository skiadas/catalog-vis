<template>
  <div v-if="item.type === 'course_group'" class="course-group">
    <template v-for="(course, ci) in item.courses || []" :key="ci">
      <span class="course-chip" @click="goToCourse(course.code)" :title="courseName(course.code)">{{
        course.code
      }}</span>
  ...
  <template v-else-if="item.type === 'course'">
    <span class="course-chip" @click="goToCourse(item.code)" :title="courseName(item.code)">{{
      item.code
    }}</span>
  ...
  <div v-else-if="eachOfInline" class="pair-group">
    <template v-for="(sub, di) in item.items || []" :key="di">
      <span class="course-chip" @click="goToCourse(sub.code)" :title="courseName(sub.code)">{{
        sub.code
      }}</span>
  ...
  <div v-else-if="item.type === 'electives'" class="electives-group">
    <div class="electives-header">
      <span class="electives-count"
        >{{ item.count ? 'Choose ' + item.count : 'Choose' }}
        {{ item.count === 1 ? 'course' : 'courses' }}</span
      >
      <span v-if="item.note" class="item-note">{{ item.note }}</span>
    </div>
        ...
        <template v-else-if="c.type === 'exclude'">
          <span class="constraint-tag exclude">
            ...
            <template v-for="(code, ei) in c.codes" :key="code">
              <span class="course-chip mini" @click="goToCourse(code)" :title="courseName(code)">{{
                code
              }}</span>
...
/**
 * The shapes this renderer accepts: the contract's requirement nodes plus the
 * browse-local synthesizers (course_group from RequirementSection's grouping,
 * pair from legacy data). Flattened with optional members so templates can
 * narrow by `item.type` and probe optional payloads with plain truthiness.
 * @typedef {{
 *   type: string,
 *   code?: import('@major-vis/catalog-contract').CourseCode,
 *   codes?: import('@major-vis/catalog-contract').CourseCode[],
 *   courses?: Array<{ code: import('@major-vis/catalog-contract').CourseCode, note?: string }>,
 *   items?: RenderedRequirement[],
 *   count?: number,
 *   min?: number,
 *   max?: number,
 *   label?: string,
 *   text?: string,
 *   note?: string,
 *   constraints?: import('@major-vis/catalog-contract').Constraint[],
 * }} RenderedRequirement
 */
...
export default {
  ...
  props: {
    item: {
      /** @type {import('vue').PropType<RenderedRequirement>} */
      type: Object,
      required: true,
    },
  },
